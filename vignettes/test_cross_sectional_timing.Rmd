...
```{r small-sample,}
# Note this has a logical error in that if an activity is not represented in 
# the sample, the code breaks because the number of observed types has to 
# match the number of activity types.
...
small_sample <- make_activity_observation("developer", size = 5 * 5 * 12, 
                                          dev_freq)
small_sample_counts <- table(small_sample)
# zeros <- rep(0.0, length(dev_activities$action))
# small_sample_counts <- ifelse(is.na(small_sample_counts), zeros,
#                               small_sample_counts)
...
description <- names(small_sample_duration)
missing_names <- dev_activities$action[
  !is.element(dev_activities$action, description)]
missing_names_len <- length(missing_names)
description <- c(description, missing_names) 
duration <- as.numeric(small_sample_duration)
duration <- c(duration, rep(0, missing_names_len))
small_sample_duration <- data.frame(description = description,
                                    duration = duration)
